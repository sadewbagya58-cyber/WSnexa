...
export const ModifierGroupSelector = React.memo(function ModifierGroupSelector({
  ...
  if (isSingle) {
    selectionRuleLabel = isRequired
      ? t('Choose 1', '1ක් තෝරන්න')
      : t('Optional (Choose 1)', 'විකල්ප (1ක් තෝරන්න)');
  } else {
    ...
    if (min > 0 && max > 0 && min !== max) {
      ...
      selectionRuleLabel = t(`Choose ${min}–${capMax}`, `${min}–${capMax} අතර තෝරන්න`);
    } else if (min > 0) {
      selectionRuleLabel = t(`Choose at least ${min}`, `අවම වශයෙන් ${min}ක් තෝරන්න`);
    } else if (max > 0) {
      const capMax = Math.min(max, availableOptions.length);
      selectionRuleLabel =
        capMax > 0
          ? t(`Choose up to ${capMax}`, `උපරිම ${capMax}ක් දක්වා තෝරන්න`)
          : t('Choose optional', 'විකල්ප තේරීම');
    } else {
      selectionRuleLabel = t('Choose optional', 'විකල්ප තේරීම');
    }
  }
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            {isRequired ? (
              <Badge variant="warning">{t('Required', 'අවශ්‍යයි')}</Badge>
            ) : (
              <Badge variant="neutral">{t('Optional', 'විකල්ප')}</Badge>
            )}
          </div>
          {group.description && (
      ...
      {group.options.length === 0 || availableOptions.length === 0 ? (
        <div className="text-xs text-zinc-500 italic p-3 bg-zinc-50 rounded-lg border border-dashed border-zinc-200">
          {t('No options currently available for this group.', 'මෙම කාණ්ඩය සඳහා විකල්ප නොමැත.')}
        </div>
      ) : (
