...
export const KitchenOrderQueue: React.FC<KitchenOrderQueueProps> = ({ initialOrders }) => {
    ...
    return (
      <div className="rounded-2xl border border-zinc-200 bg-white p-12 text-center space-y-3 shadow-2xs">
        ...
        <p className="text-xs text-zinc-500 max-w-sm mx-auto">
          There are currently no active pending or preparing orders for this branch. New guest orders will appear here automatically.
        </p>
        <Button variant="outline" className="text-xs font-bold mt-2" onClick={() => router.refresh()}>
          🔄 Refresh Queue
        </Button>
      </div>
    );
  ...
  const statusMap: Record<OrderStatus, { title: string; badge: 'warning' | 'success' | 'neutral' | 'destructive'; icon: string }> = {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="text-xs font-extrabold uppercase tracking-wider text-zinc-500">
          Active Kitchen Orders ({initialOrders.length})
        </div>
        <Button variant="outline" className="text-xs font-bold" onClick={() => router.refresh()} disabled={isPending}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {initialOrders.map((order) => {
          const statusInfo = statusMap[order.status] || { title: order.status, badge: 'neutral', icon: '📦' };
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2 pt-1">
                  {order.items?.map((item) => (
                    <div key={item.id} className="text-xs space-y-0.5 border-b border-zinc-50 pb-2 last:border-0">
                      <div className="flex items-start justify-between font-bold text-zinc-950">
                        <span className="flex items-center gap-1.5">
                          <span className="font-mono text-zinc-500 text-[11px]">{item.quantity}x</span>
