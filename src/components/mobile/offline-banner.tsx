...
import { SyncStatusSheet } from './sync-status-sheet';
...
export const OfflineBanner: React.FC<OfflineBannerProps> = ({
  ...
  if (!isOffline && !isSyncing && !hasPending && !hasErrors) {
    return <SyncStatusSheet isOpen={internalSheetOpen} onClose={() => setInternalSheetOpen(false)} />;
  }
  ...
  return (
    <>
      <div
        ...
        onClick={() => {
          if (hasErrors || onOpenSyncDetails) {
            openDetails();
          }
        }}
        style={{
          paddingTop: 'calc(max(env(safe-area-inset-top, 0px), var(--sat, 0px)) + 0.625rem)',
        }}
        className={`sticky top-0 z-50 w-full px-4 pb-2.5 transition-all text-xs font-medium shadow-sm flex items-center justify-between ${
        ...
        <div className="flex items-center gap-2">
          {hasErrors && (
            <button
              ...
              className="px-2.5 py-1 rounded bg-white text-rose-700 hover:bg-rose-50 text-[11px] font-black shadow-xs transition active:scale-95 touch-manipulation cursor-pointer"
            >
              Review ({stats.failed_count + stats.conflict_count})
            </button>
          )}
          ...
          {hasPending && !isSyncing && !isOffline && (
            <button
              ...
              className="px-2 py-0.5 rounded bg-white/20 hover:bg-white/30 text-white text-[11px] font-semibold transition touch-manipulation"
            >
              Sync Now
            </button>
          )}

          {!hasErrors && (onOpenSyncDetails || isOffline || hasPending) && (
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
              ...
              className={`px-2 py-0.5 rounded underline text-[11px] font-medium transition touch-manipulation cursor-pointer ${
      ...
      <SyncStatusSheet isOpen={internalSheetOpen} onClose={() => setInternalSheetOpen(false)} />
    </>
  );
};
