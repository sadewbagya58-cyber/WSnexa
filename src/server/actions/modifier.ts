...
export async function createModifierGroupAction(
  ...
  const itemResource = { type: 'menu_item' as const, id: formData.menuItemId };
  const canManage =
    (await can({ context: authContext, permission: 'menu.modifiers.manage', resource: itemResource })) ||
  ...
  // Verify menu item exists and belongs to active business & branch
...
export async function updateModifierGroupAction(
  ...
  const canManage =
    (await can({ context: authContext, permission: 'menu.modifiers.manage', resource: branchResource })) ||
    (await can({ context: authContext, permission: 'menu.manage', resource: branchResource }));

  if (!canManage) return { success: false, message: 'Forbidden. Missing permission to manage modifiers.' };
