...
// Canonical Server Actions for SaaS Subscription Payments (Owner History & Admin Management)
...
export interface CancelOwnerPaymentIntentInput {
  paymentId: string;
}

export interface CancelAdminPaymentIntentInput {
  paymentId: string;
  reason: string;
}
...
export async function cancelOwnerPendingPaymentIntentAction(input: CancelOwnerPaymentIntentInput) {
  try {
    ...
    if (intent.business_id !== authContext.businessId) {
    ...
    const { data: updated, error: updateErr } = await admin
      ...
      .update({
        status: 'cancelled',
        cancelled_at: now,
        admin_reason: null, // Owner cancellation leaves admin_reason NULL
        updated_at: now,
      })
      .eq('id', intent.id)
      .select('*')
